...
MarkdownHighlighter::MarkdownHighlighter(
    ...
    : TextEditor::SyntaxHighlighter(parent), _highlightingOptions(highlightingOptions) {
...
void MarkdownHighlighter::highlightInlineSpans(const QString &text,
    ...
    for (int i = currentPos; i < text.length(); ++i) {
        ...
        QTextCharFormat fmt = TextEditor::SyntaxHighlighter::format(start + 1);
    ...
    for (int i = delims.length() - 1; i >= 0; --i) {
        ...
        if (isStrong) {
            ...
            while (k != (startDelim.pos + boldLen)) {
                QTextCharFormat fmt = TextEditor::SyntaxHighlighter::format(k);
