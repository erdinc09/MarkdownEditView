...
void MarkdownHighlighter::initTextFormats(int defaultFontSize) {
  ...
  _formats[HorizontalRuler] =
      fonts.toTextCharFormat({Style::C_CURRENT_LINE, {}});

  // set character format for lists
  _formats[List] = fonts.toTextCharFormat({Style::C_NUMBER, {}});
  ...
  _formats[Image] = fonts.toTextCharFormat({Style::C_OCCURRENCES, {}});

  // set character format for code blocks
  format = fonts.toTextCharFormat({Style::C_AUTOCOMPLETE, {}});
  ...
  _formats[Comment] = fonts.toTextCharFormat({Style::C_COMMENT, {}});

  // set character format for masked syntax
  _formats[MaskedSyntax] = fonts.toTextCharFormat({Style::C_KEYWORD, {}});

  // set character format for tables
  _formats[Table] = fonts.toTextCharFormat({Style::C_TEXT, {}});

  // set character format for block quotes
  _formats[BlockQuote] = fonts.toTextCharFormat({Style::C_OCCURRENCES, {}});
...
void MarkdownHighlighter::highlightHeadline(QString text) {
  ...
  if (match.hasMatch()) {
    ...
    setFormat(match.capturedStart(2), match.capturedLength(2), _formats[state]);
...
void MarkdownHighlighter::highlightCodeBlock(QString text) {
  ...
  if (match.hasMatch()) {
    setCurrentBlockState(previousBlockState() == HighlighterState::CodeBlock
                             ? HighlighterState::CodeBlockEnd
